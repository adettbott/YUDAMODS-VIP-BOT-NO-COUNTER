...
module.exports.welcome = async(iswel, isleft, yudamods, anu) =>{
	try {
            const metadata = await yudamods.groupMetadata(anu.id)
            ...
            for (let num of participants) {
  		      const fkontaku = { key: {participant: `0@s.whatsapp.net`, ...(anu.id ? { remoteJid: `6283842204546-1614953337@g.us` } : {}) }, message: { 'contactMessage': { 'displayName': ``, 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;,;;;\nFN:,\nitem1.TEL;waid=${num.split('@')[0]}:${num.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, 'jpegThumbnail': setting.pathimg, thumbnail: setting.pathimg,sendEphemeral: true}}}
                try {
                    pp_user = await yudamods.profilePictureUrl(num, 'image')
                } catch {
    ...
    var wc = await getBuffer(`https://telegra.ph/file/eaf642e46f00344e93a2c.png`)
                	await yudamods.sendMessage(anu.id, {image: wc, caption: `Hii @${num.split("@")[0]},Welcome In ${groupName} Jangan Lupa Intro! \n${groupDesc}`, mentions: [num]}, {quoted:fkontaku})
                }
                	...
                	if (isSetLeft(anu.id, set_left_db)) {            	
                        ...
                        var full_pesan = (replace_pesan.replace(/@group/gi, groupName).replace(/@desc/gi, groupDesc))
                      yudamods.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `${full_pesan}` })
                       } else {
   var lf = await getBuffer(`https://telegra.ph/file/a0a6f7bc72d1a6be0fa48.png`) 
await yudamods.sendMessage(anu.id, {image: lf, caption: `Sayonara @${num.split("@")[0]}`, mentions: [num]}, {quoted:fkontaku})
                        } 
                } else if (anu.action == 'promote') {
                  yudamods.relayMessage(anu.id, {
	...
	module.exports.aDelete = async (setting, yudamods, m) => {
...
✍️ *MessageType* : ${mek.mtype}
    `,
		...
		await yudamods.copyNForward(froms, mek, true);
  	await delay(4000)
...
module.exports.oneTime = async (setting,yudamods,m) => {
...
await yudamods.sendTextWithMentions(m.chat, teks, m)
  await delay(500)
